...
chang_country_btn.addEventListener("click", function(){
    ...
    //adding animation to click change
...
//after clicking country, box disappear
country_list_element.addEventListener("click", function(){
    search_country_element.classList.toggle("hide");
});

// COUNTRY FILTER 
/* input event files up whenever the value of the input changes */
input.addEventListener("input", function(){
    let value = input.value.toUpperCase();

    country_list.forEach( country => {
        //if input matches country then remove hide for them
        if( country.name.toUpperCase().startsWith(value)){
            document.getElementById(country.name).classList.remove("hide");
        }
        //add hide to all other countries
        else{
